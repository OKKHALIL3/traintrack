...
2. **Spawn** one worker per part: `spawn_worker(agent, role, task)`. Each worker runs **headless in its own git worktree** and replies automatically — these are your truly-hands-off teammates (unlike live sessions, they auto-receive instantly).
