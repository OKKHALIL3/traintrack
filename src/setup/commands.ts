...
- "spawn <description>" → Call \`spawn_worker(agent, role, task, model?)\` to start ONE worker for the task. Choose agent = "claude" or "codex" for the task, pick a short role, pass a clear task; optionally pass a model (e.g. "haiku") to pin the worker's model. Report the new worker's handle.
