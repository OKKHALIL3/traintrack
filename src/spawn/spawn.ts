...
export interface BuildWorkerCommandOptions {
  ...
  handle: string
  channel: string
  /** Model the worker passes to its agent CLI on every turn (omit → provider default). */
  model?: string
}
...
export interface SpawnWorkerOptions {
  ...
  role: string
  task: string
  /** Model forwarded to the worker's agent CLI (omit → provider default). */
  model?: string
  leadHandle?: string
...
export function buildWorkerCommand(opts: BuildWorkerCommandOptions): BuildWorkerCommandResult {
  const { agent, role, handle, channel, model } = opts
  const cliPath = resolveCliPath()
  const args = [cliPath, 'worker', '--agent', agent, '--role', role, '--handle', handle, '--channel', channel]
  if (model) {
...
export async function spawnWorker(opts: SpawnWorkerOptions): Promise<SpawnWorkerResult> {
  ...
  const { command, args } = buildWorkerCommand({ agent, role, handle, channel: channel.dbPath, model })
