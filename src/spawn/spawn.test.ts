...
describe('buildWorkerCommand', () => {
  ...
  it('appends --model when a model is given', () => {
    const result = buildWorkerCommand({ agent: 'claude', role: 'api', handle: 'worker_abc', channel: '/tmp/ch.db', model: 'haiku' })
    const i = result.args.indexOf('--model')
    expect(i).toBeGreaterThan(-1)
    expect(result.args[i + 1]).toBe('haiku')
  })

  it('omits --model when no model is given', () => {
    const result = buildWorkerCommand({ agent: 'claude', role: 'api', handle: 'worker_abc', channel: '/tmp/ch.db' })
...
describe('spawnWorker', () => {
  ...
  it('forwards the model into the spawned worker argv', async () => {
    const execFileImpl = (_cmd: string, _args: string[], _opts: unknown, cb: (err: Error | null) => void) => cb(null)
    const mockProcess = { unref: vi.fn(), stdout: null, stderr: null }
    const spawnImpl = vi.fn().mockReturnValue(mockProcess)
    ...
    const workerArgs = spawnImpl.mock.calls[0][1] as string[]
    const i = workerArgs.indexOf('--model')
    expect(i).toBeGreaterThan(-1)
    expect(workerArgs[i + 1]).toBe('haiku')
  })
