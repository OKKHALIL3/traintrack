    ...
    inputSchema: {
      ...
      properties: {
        ...
        model: {
          type: 'string',
          description:
            'Optional: the model the worker uses for every turn, passed straight to its agent CLI — e.g. "sonnet", "opus", or "haiku" for claude, or a codex/cursor/opencode model id. Omit to use the provider default.',
        },
      },
...
export async function spawnWorkerTool(
  ...
  // Model is a pass-through: the provider CLI validates its own model names, so
  // new models work without a traintrack update. Blank → provider default.
  ...
  return textResult(
    `Spawned ${agent} worker (${role})${model ? ` on model ${model}` : ''} as ${handle}. Call await_results to collect their reply.`
  )
}
