...
describe('spawnWorkerTool', () => {
  ...
  it('forwards the model to spawnWorker and mentions it in the text', async () => {
    const c = makeChannel()
    const spawnWorker = fakeSpawn('worker_abc123')
    const res = await spawnWorkerTool(
      { agent: 'claude', role: 'api', task: 'do X', model: 'haiku' },
      deps(c, { spawnWorker })
    )
    expect(spawnWorker.mock.calls[0][0]).toMatchObject({ agent: 'claude', model: 'haiku' })
    expect(res.content[0].text).toContain('haiku')
    c.close()
  })

  it('passes model undefined when omitted or blank', async () => {
    const c = makeChannel()
    const spawnWorker = fakeSpawn()
    await spawnWorkerTool({ agent: 'claude', role: 'api', task: 'do X' }, deps(c, { spawnWorker }))
    expect(spawnWorker.mock.calls[0][0].model).toBeUndefined()
    await spawnWorkerTool({ agent: 'claude', role: 'api', task: 'do X', model: '  ' }, deps(c, { spawnWorker }))
    expect(spawnWorker.mock.calls[1][0].model).toBeUndefined()
    c.close()
  })
})
